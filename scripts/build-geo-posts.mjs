// build-geo-posts.mjs
// Reads GEO article markdown files from opc-doc, extracts frontmatter + JSON-LD +
...
function parseCta(rest) {
  ...
  if (!line) return ''
  const text = line.replace(/\[WAITLIST_URL\]/g, `<a href="${PROD_URL}" class="underline font-semibold">try AIActRadar</a>`)
  return '<p>' + inline(text) + '</p>'
}

function fixDomain(obj) {
  return JSON.parse(JSON.stringify(obj).replace(/https:\/\/aiactradar\.com/g, LIVE_DOMAIN))
}
...
for (const f of files) {
  ...
  const description = (blogPosting.description || '').replace(/https:\/\/aiactradar\.com/g, LIVE_DOMAIN)
  posts.push({
    slug: fm.slug,
    title: fm.title || blogPosting.headline || f,
    datePublished: fm.date || blogPosting.datePublished || '2026-07-25',
    dateModified: blogPosting.dateModified || fm.date || '2026-07-25',
    description,
    html: mdToHtml(body).replace(/https:\/\/aiactradar\.com/g, LIVE_DOMAIN),
    faq: parseFaq(faq),
...
const ts = `// AUTO-GENERATED by scripts/build-geo-posts.mjs — do not edit by hand.
