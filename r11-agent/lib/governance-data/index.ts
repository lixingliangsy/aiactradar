...
function resolveDataDir(): string {
  const candidates = [
    path.join(process.cwd(), 'r11-agent', 'lib', 'governance-data'),
    path.join(process.cwd(), 'lib', 'governance-data'),
    __dirname,
  ]
  for (const dir of candidates) {
    if (fs.existsSync(path.join(dir, 'eu-ai-act.json'))) return dir
  }
  return candidates[0]
}
...
function readJson(name: string): any {
  ...
  if (!fs.existsSync(p)) {
    console.warn(`[governance-data] Warning: data file not found: ${p}`)
    return null
  }
